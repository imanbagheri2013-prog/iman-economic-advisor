...
def _risk_assessment(report: dict[str, Any], policy: RiskPolicy = DEFAULT_RISK_POLICY) -> tuple[int, list[str]]:
    ...
    if news_regime is None:
        score += policy.unavailable_news_points; flags.append("news_risk_unavailable")
    elif news_regime == "HIGH_RISK":
        score += policy.high_news_risk_points; flags.append("high_news_risk")
    elif news_regime == "ELEVATED_RISK":
        score += policy.elevated_news_risk_points; flags.append("elevated_news_risk")
    imbalance = (by_name.get("liquidity", {}).get("details") or {}).get("depth_imbalance")
    if imbalance is None:
        score += policy.unavailable_liquidity_points; flags.append("liquidity_unavailable")
    elif abs(float(imbalance)) >= policy.extreme_liquidity_imbalance:
        score += policy.extreme_liquidity_points; flags.append("extreme_liquidity_imbalance")
    ...
    if funding_rate is not None and abs(float(funding_rate)) >= policy.extreme_funding_rate_pct:
        score += policy.extreme_funding_points; flags.append("extreme_funding_crowding")
    ...
    if isinstance(monetary_impulse, dict):
        ...
        if points:
            score += points; flags.append(f"cbi_monetary_{direction.lower()}")
...
def build_decision(report: dict[str, Any], policy: RiskPolicy = DEFAULT_RISK_POLICY) -> dict[str, Any]:
    ...
    risk_tier = _risk_tier(risk_score, policy); exposure_multiplier = _exposure_multiplier(risk_tier, policy)
    stale_market_data = "stale_market_data" in risk_flags
    if score is None or coverage < policy.minimum_coverage or stale_market_data or "high_news_risk" in risk_flags or "liquidity_unavailable" in risk_flags or risk_score >= policy.critical_score:
        action = "NO_TRADE"
