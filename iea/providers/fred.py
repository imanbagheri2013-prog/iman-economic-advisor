...
class FRED:
    ...
    def _api_observations(self, series_id, limit):
        ...
        for item in response.json().get("observations", []):
            ...
            out.append(Observation(provider="fred", series_id=series_id, date=date, value=value, retrieved_at=now, quality=quality(value, now), status="OK" if value is not None else "MISSING"))
        return out

    def _public_csv_observations(self, series_id, limit):
        response = requests.get(PUBLIC_CSV_URL, params={"id": series_id}, timeout=REQUEST_TIMEOUT_SECONDS)
        response.raise_for_status()
