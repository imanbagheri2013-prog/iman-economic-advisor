...
def analyze_snapshot(snapshot: MarketSnapshot) -> MarketSignal:
    """Produce a conservative signal from one normalized market snapshot.

    The safety rule is intentional: missing or stale market data can never
    become an actionable BUY/SELL signal.
    """
    ...
    if not snapshot.fresh:
        return MarketSignal(
            ...
            confidence=0.0,
            market_state="STALE",
            reasons=("market snapshot is stale",),
            data_fresh=False,
    ...
    if change is None:
        return MarketSignal(
            ...
            reasons=("previous close is missing or invalid",),
            data_fresh=True,
            observed_at=observed.isoformat(),
            source=snapshot.source,
        )
